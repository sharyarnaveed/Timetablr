...
export const adminverifyjwt = async (req, res, next) => {
    try {
        const atoken = req.cookies?.adminaccessToken || req.header("Authorization")?.replace("Bearer ", "");
        ...
        if (!atoken) {
            ...
            if (!retoken) {
                ...
                try {
                    ...
                    const newacctoken = jwt.sign({ id: result.admin_id}, process.env.ACCESS_TOKEN_KEY, { expiresIn: process.env.ACCESS_TIME });
                    const theop = {
                        httpOnly: true,
